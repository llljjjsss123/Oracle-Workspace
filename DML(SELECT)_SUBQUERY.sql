...
-- 2) 평균급여(3047663)보다 많은 급열르 받고있는 사원조회
...
/*
    ...
    => 서브쿼리를 수행한 결과가 몇행 몇열이냐에 따라 사용가능한 연산자가 달란진다.
*/
/*
     ...
     OR (JOB_CODE='J3' AND SALARY='3400000')
      OR (JOB_CODE='J4' AND SALARY='1550000')
       ...
       OR (JOB_CODE='J6' AND SALARY='2000000')
         OR (JOB_CODE='J1' AND SALARY='1380000');
--3. 위 내용을 가지고 하나의 커리문으로 만들기
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE(JOB_CODE,SALARY)IN((SELECT JOB_CODE, MIN(SALARY)
FROM EMPLOYEE
GROUP BY JOB_CODE));
--각 부서 별로  최고 급여를 받는 사원들 조회(사번, 이름, 부서코드, 급여)
...
--인라인 뷰 사용: 사원명만 골라내기 (보너스포함 연봉이 3000만원 이항신 사원들)
SELECT EMP_NAME
FROM (SELECT EMP_ID, EMP_NAME,(SALARY+SALARY*NVL(BONUS,0))*12 보너스포함연봉, DEPT_CODE
FROM EMPLOYEE
WHERE (SALARY+SALARY*NVL(BONUS,0))*12>=30000000
)
WHERE DEPT_CODE IN NULL;
--인라인뷰를 주로 사용하는 예
--TOP-N 분석:데이터베이스상에 있는 자료 중 최상위 N개의 자료를 보기 위해 사용하는 기능.
--전 직원중 급여가 가장 높은 상위 5명(순위,사원명,급여)
--*ROWNUM:오라클에서 자동으로 제공해주는 칼럼."조회된" 순서대로 1부터 순번을 부여해주는 컬럼.
SELECT ROWNUM, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC;
--원하는 값이 안나온 이유 :ORER BY로 정렬하기전에 이미 ROWNUM의 순번이 매겨져 있기 때문
--해결방법:ORDER BY로 이미 정렬한 테이블을 가지고 ROWNUM순번을 매기면됨.
SELECT ROWNUM, EMP_NAME, SALARY
FROM(
    SELECT * 
    FROM EMPLOYEE
...
--1)각 부서별 평균 급여가 높은 3개 정렬
SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY 2 DESC;
--2)순번부여, 가장 많이 받는 3개부서만 추리기.
SELECT ROWNUM, S.*--DEPT_CODE, SALARY
FROM (
    SELECT DEPT_CODE, ROUND(AVG(SALARY))
    FROM EMPLOYEE
    GROUP BY DEPT_CODE
...
--다만, 정렬이 되지 않은 상태에서는 순위를 매겨도 의미가 없으므로 선정렬하고 순위를 매겨야한다
--우선적으로 인라인뷰로ORDER BY절을 만들고 그후에 메인 쿼리에서 순번을 붙임

-- 최근에 입사한 사원 5명 (사원, 급여, 입사일)
...
-DENSE_RANL() OVER(정렬기준) :공동 1위가 3명이면 그 다음 순위는 2위로.
정렬기준: ORDER BY절(정렬기준 칼럼이름, 오름차순,/내림차순,NULLFIRST나 NULLSLAST 기술불가.

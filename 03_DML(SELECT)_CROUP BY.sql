...
SELECT DEPT_CODE , SUM(SALARY) AS 총급여--4.셀렉트절 실행
FROM EMPLOYEE--1.FROM절이 먼저
WHERE 1=1--2.WHERE 두번째
GROUP BY DEPT_CODE--3.그룹바이절 실행
...
SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여자')성별 , COUNT(*)사원수
FROM EMPLOYEE
...
SELECT JOB_CODE,ROUND(AVG(SALARY)),MAX(SALARY),MIN(SALARY),COUNT(*)
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING ROUND(AVG(SALARY))>=3000000;
...
1.FROM 조회하고자 하는 테이블명/인라인/가상테이블(DUAL)
2.WHERE 조건식(그룹함수를 사용할수 없음.
3.GROUP BY 그룹 기준에 해당하는 칼럼명/함수식
4.HAVING 그룹함수식에 대한 조건식
6.ORDER BY (정렬기준에 해당하는 칼럼명/별칭/컬럼의 순번)[ASC/DESC]생략가능[NULL FIRST/NULLS LAST](생략가능)

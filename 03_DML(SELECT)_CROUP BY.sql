...
SELECT JOB_CODE, SUM(SALARY) AS 총급여합
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING SUM(SALARY) >= 10000000;
-- 2. 각 직급별 급여 평균이 300만원 이상인 직급코드, 평균급여, 사원수, 최고급여, 최소급여
SELECT JOB_CODE, AVG(SALARY), COUNT(*) , MAX(SALARY), MIN(SALARY)
FROM EMPLOYEE 
GROUP BY JOB_CODE 
HAVING AVG(SALARY) >= 3000000;
-- 3. 각 부서 별 보너스를 받는 사원이 없는 부서만 조회
SELECT DEPT_CODE
FROM EMPLOYEE 
GROUP BY DEPT_CODE
...
-- 직급코드가 J6이거나 또는 부서코드가 D1원 사원들을 조회 (사번, 사원명, 부서코드, 직급코드) => UNION을 사용해서 데이터조회.
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE JOB_CODE = 'J6'
UNION
SELECT EMP_ID, EMP_NAME, DEPT_CODE, '1' AS JOB_CODE
FROM EMPLOYEE
WHERE DEPT_cODE ='D1';
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE JOB_CODE = 'J6' -- 정현돈, 장쯔위, 하도운 ,차태연 ,전지연, 이태림
MINUS
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE DEPT_cODE ='D1'; -- 방명수 , 차태연 ,전지연

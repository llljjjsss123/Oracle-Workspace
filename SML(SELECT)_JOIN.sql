/*
    ...
    두개 이상의 테이블에서 데이터를 같이 조회하고자 할 때 사용되는 구문 => SELECT 문ㅇ 이용 
    조회 결과는 하나의 결과물(RESULT SET)으로 나옴
    
    JOIN을 해야하는 이유 ?
    관계형 데이터베이스에서는 최소한의 데이터로 각각의 테이블에 데이터를 보관하고 있음.
    사워정보는 사원테이블, 직급정보는 직급테이블, ... => 중복을 최소화하기 위함.
...
/*
    ...
    JOIN 조인할 테이블명 1개만제시 USING(연결할 컬럼명 1개만 제시)
...
WHERE DEPT_CODE = DEPT_ID; -- EMPLOYEE테이블의 DEPT_cODE값과 DEPARTMENT테이블의 DEPT_ID이 일치하는경우에만 조회.
--> 일치하지 않는 값은 조회되지 않는다. (부서코드가 NULL인 사원 , )
--> 두개 이상의 테이블을 조인할때 일치하는값이 없는행은 결과에서 제외됨.

-- 전체 사원들의 사번, 사원명, 직급코드, 직급명을 알아내고자한다면?
-- 연결할 칼러명이 일치(EMPLOYEE - JOB_CODE/ JOB - JOB_CODE)
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE , JOB 
WHERE JOB_CODE = JOB_CODE; -- 에러발생 : column ambiguously defined : 컬럼명이 애매모호 하다. -> 확실히 어디테이블의 
-- 칼럼인지를 제시해줘야함.
...
-- 그 뒤에JOIN절에서 같이 조회하고자 하는 테이블 기술. 또한 매칭시킬 컬럼에 대한 조건도 같이 기술.
-- USING / ON 구문

-- 전체 사원들의 사번, 사원명, 부서코드 , 부서명을 알아내고자 할때
-- 1) 연결할 컬럼명 이 다른경우( EMPLOYEE - DEPT_CODE / DEPARTMENT - DEPE_ID)
--    => 무조건 ON구문만 가능.
SELECT EMP_ID , EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
...
-- ==> ON, USING구문 둘다 사용가능함.
-- 2-1) ON 구문 : ambiguously가 발생할수 있기때문에 칼럼명을 명확하게 기술해줘야함.
SELECT EMP_ID , EMP_NAME, E.JOB_CODE, JOB_NAME
FROM EMPLOYEE E 
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE);

-- 2-2) USING구문 : 컬럼명이 동일한 경우에만 사용 가능.
--                   동일한 컬럼명 하나만써주면 알아서 매칭시켜줌.
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE E 
...
FROM EMPLOYEE 
NATURAL JOIN JOB;
-- 운좋게도 두개의테이블에 일치하는 칼럼명이 "딱 한개"만 존재(JOB_CODE) -> 자연조건으로 사용시 알아서 JOB_CODE.가 매칭됨.
...
-- 4. 아래의 두테이블을 참고해서 부서코드, 부서명, 지역코드 ,지역명 조회
-- 오라클전용구문, ANSI구문 2가지로 표현
...
FROM EMPLOYEE 
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
-- DEPT_CODE가 NULL인사원 두명은 조회되지 않음. 즉, "모든" 사원들의 정보가 출력되는게 아님.
...
--                      즉 , 뭐가되었뜬 왼편에 기술된 테이블의 데이터는 무조건 조회되게끔 한다.
--                      (조인하는  테이블과일치하는 값이 없더라도)

--> ANSI구문
SELECT EMP_NAME, SALARY , DEPT_TITLE
FROM EMPLOYEE LEFT /* OUTER */ JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
-- EMPLOYEE테이블을 기준으로 조회했기 때문에 EMPLOYEE에 존재하는 데이터는 뭐가 되었뜬 다 조회가능하게끔 해줌.
...
/*
    ...
    각각 N개, M개의 행을가진 테이블이 있다면 출력결과가 되는 행의 갯수는  N * M행
...
FROM EMPLOYEE, DEPARTMENT
ORDER BY EMP_NAME ASC; -- 23 인원 * 9개의 부서 -> 207개 행 조회
